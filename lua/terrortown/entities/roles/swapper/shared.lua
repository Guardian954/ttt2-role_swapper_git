...
if SERVER then
	...
	hook.Add("TTTBeginRound", "GrabAllSwappers", function()
		local players = player.GetAll()	
		local swapperTable = {}
		for i = 1, #players do
			local ply = players[i]
			if ply:GetSubRole() == ROLE_SWAPPER then
				swapperTable[#swapperTable + 1] = ply
			end
		end
		hook.Add("TTT2JesterModifyList", "AddSwapperToJesterList", function(table)
			local jesterTable = table
			for i = 1, #swapperTable do
				local swapper = swapperTable[i]
				jesterTable[#jesterTable + 1] = swapper
			end
		end)
	end)
